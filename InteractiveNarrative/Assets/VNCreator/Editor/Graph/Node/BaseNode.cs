...
namespace VNCreator
{
    ...
    public class NodeViewer : VisualElement
    {
        ...
        public NodeViewer(BaseNode _node)
        {
            ...
            TextField GoodOrBad = this.Query<TextField>("Good_Bad");
            GoodOrBad.value = node.nodeData.GoodOrBad.ToString();
            GoodOrBad.RegisterValueChangedCallback(
                e =>
                {
                    int newValue;
                    if (int.TryParse(e.newValue, out newValue))
                    {
                        node.nodeData.GoodOrBad = newValue;
                    }
                }
            );
